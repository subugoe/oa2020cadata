...
tt_oa <- my_df %>%
  ...
  summarise(n = sum(articles)) %>%
  mutate(oa_journal = ifelse(oa_journal == TRUE, "in Fully Open Access Journals", "in Subscription Journals")) %>%
  ungroup() %>%
  group_by(oa_journal) %>%
  mutate(prop = n / sum(n))
plot_df <- tt_all %>%
  mutate(oa_journal = "Overall Article Volume") %>%
  bind_rows(tt_oa) %>%
  group_by(oa_journal) %>%
