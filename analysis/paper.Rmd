...
tt_oa <- my_df %>%
  ...
  summarise(n = sum(articles)) %>%
  mutate(oa_journal = ifelse(oa_journal == TRUE, "Full Open Access", "Subscription")) %>%
  ungroup() %>%
  group_by(oa_journal) %>%
  mutate(prop = n / sum(n))
plot_df <- tt_all %>%
